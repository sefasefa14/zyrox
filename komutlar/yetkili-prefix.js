...
let p = await db.fetch(`prefix.${message.guild.id}`) || ayarlar.prefix
 ...
 if (!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send(new Discord.RichEmbed().setColor("RED").setDescription(`<a:zyroxcarpi:768471338590535770> Bu Komutu Kullanabilmek İçin Mesajları Yönet Yetkisine Sahip Olmalısınız.\nŞuanki Prefix: ${p}`));
